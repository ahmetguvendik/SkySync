...
using IdentityIOutboxRepository = SkySync.Services.Identity.Application.Interfaces.IOutboxRepository;
using IdentityIUnitOfWork = SkySync.Services.Identity.Application.UnitOfWorks.IUnitOfWork;
using SkySync.Shared.OutboxTable;
using SkySync.Workers.Outbox.Jobs.Common;

namespace SkySync.Workers.Outbox.Jobs.Identity;

/// <summary>
/// Identity Service Outbox Worker - Yeni kullanıcı kayıt eventlerini RabbitMQ'ya publish eder.
/// </summary>
public class IdentityOutboxPublishWorker : BackgroundService
{
    private readonly IServiceProvider _serviceProvider;
    private readonly ILogger<IdentityOutboxPublishWorker> _logger;

    public IdentityOutboxPublishWorker(IServiceProvider serviceProvider, ILogger<IdentityOutboxPublishWorker> logger)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        _logger.LogInformation(
            "Identity Outbox Worker başlatıldı. Her {DelaySeconds} saniyede bir kontrol edilecek. MaxRetry: {MaxRetryCount}",
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                var outboxRepository = scope.ServiceProvider.GetRequiredService<IdentityIOutboxRepository>();
                var unitOfWork = scope.ServiceProvider.GetRequiredService<IdentityIUnitOfWork>();
                ...
                if (processedCount > 0 || failedCount > 0 || permanentlyFailedCount > 0)
                {
                    ...
                    _logger.LogInformation(
                        "Identity Outbox işlemi tamamlandı. Başarılı: {ProcessedCount}, Retry Gereken: {FailedCount}, Kalıcı Başarısız: {PermanentlyFailedCount}, Toplam: {TotalCount}",
                        processedCount, failedCount, permanentlyFailedCount, messagesToProcess.Count);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Identity Outbox mesajları yayınlanırken bir hata oluştu.");
            }
    ...
    private async Task<MessageProcessResult> ProcessMessageAsync(
        ...
        IdentityIOutboxRepository outboxRepository,
